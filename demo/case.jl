...
"""
Creation of the nodes for the Agder region.
"""
function agder_nodes(𝒯)

    # Definition of investment parameters
...
"""
Creation of the nodes for the North Sea region.
"""
...
""" 
Creation of the nodes for the Danmark region, var_opex_src [€/GWh]
"""
...
""" 
Creation of the combined model.
"""
function get_data(var_opex_src, discount_rate=0.05)
...
"""
function run_case_model(optimizer, case)
...
function run_demo(prices = [0, 1000, 10000, 50000, 75000, 100000, 150000, 200000, 250000, 300000, 350000, 400000, 450000, 500000];optimizer)
    ...
    for var_opex_src in prices
        ...
        case = get_data(var_opex_src, discount_rate)

        m = run_case_model(optimizer, case)
        ...
        println()
        println("source ",  sum(value.(m[:cap_use][source, t]) for t in 𝒯))
        println("sink ",    sum(value.(m[:cap_use][sink, t]) for t in 𝒯))
        println("wind ",    sum(value.(m[:cap_use][wind, t]) for t in 𝒯))
        println("hydro ",   sum(value.(m[:stor_rate_use][hydro, t]) for t in 𝒯))
        println()
        @show value.(m[:cap_add])
        
        # Calculate the total investments in offshore wind.
        push!(investments, sum(value.(m[:cap_add][wind, t]) for t ∈ 𝒯ᴵⁿᵛ))
